...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
    @php
        $cards = [
            ...
            ["label" => "Hadir Hari Ini", "value" => $hadirHariIni, "sub" => "sudah absen masuk", "color" => "green", "icon" => "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"],
            ["label" => "Terlambat", "value" => $terlambatHariIni, "sub" => "melewati toleransi", "color" => "amber", "icon" => "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"],
...
<div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-2">
            @forelse($aktivitas as $a)
                <div class="flex items-center gap-4 rounded-2xl border border-stone-100 bg-white px-4 py-3 transition hover:border-amber-200 hover:bg-amber-50/40">
                    <div class="flex h-11 w-11 items-center justify-center rounded-xl text-sm font-bold text-white" style="background: linear-gradient(135deg, {{ $a->status_kehadiran === "terlambat" ? "#f59e0b, #d97706" : "#10b981, #059669" }});">
                    ...
                    <div class="text-right">
                        <div class="font-mono text-sm font-bold text-stone-700">{{ \Carbon\Carbon::parse($a->jam_masuk)->format("H:i") }}</div>
    ...
    <div class="space-y-6">
        <div class="relative overflow-hidden rounded-3xl p-6 text-white shadow-lg" style="background: linear-gradient(150deg, #3d2417, #5a3620);">
            ...
            <div class="relative">
                <div class="flex items-center gap-2 text-amber-300/90">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-wide">Pengeluaran Bulan Ini</span>
                </div>
        ...
        <div class="rounded-3xl border border-white/70 bg-white/80 p-6 shadow-sm backdrop-blur">
            <h3 class="mb-4 text-sm font-extrabold text-stone-800">Ringkasan Kehadiran</h3>
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2.5"><span class="h-2.5 w-2.5 rounded-full bg-green-500"></span><span class="text-sm text-stone-600">Sudah absen</span></div>
                    <span class="text-sm font-bold text-stone-800">{{ $hadirHariIni }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2.5"><span class="h-2.5 w-2.5 rounded-full bg-stone-300"></span><span class="text-sm text-stone-600">Belum absen</span></div>
                    <span class="text-sm font-bold text-stone-800">{{ max($belumAbsen, 0) }}</span>
                </div>
                @php $persen = $karyawanAktif > 0 ? round($hadirHariIni / $karyawanAktif * 100) : 0; @endphp
                <div class="pt-2">
                    <div class="mb-1.5 flex justify-between text-xs"><span class="text-stone-400">Tingkat kehadiran</span><span class="font-bold text-amber-700">{{ $persen }}%</span></div>
                    <div class="h-2.5 overflow-hidden rounded-full bg-stone-100">
                        <div class="h-full rounded-full transition-all" style="width: {{ $persen }}%; background: linear-gradient(90deg, #f59e0b, #d97706);"></div>
                    </div>
                </div>
            </div>
        </div>
